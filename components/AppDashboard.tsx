...
const AppDashboard: FC<AppDashboardProps> = ({ username, onLogout }) => {
  ...
  const [contacts, setContacts] = useState<
    { id: string; name: string; status?: string; time?: string }[]
  >([]);
    ...
    try {
      ...
      if (response.data) {
        if (Array.isArray(response.data)) {
          ...
          (response.data as any).favorites &&
          Array.isArray((response.data as any).favorites)
        ) {
  ...
  const setupSocketListeners = () => {
    socketService.on(
      "incoming-call",
      async ({ callId, callerId, callerName }: any) => {
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2">
              ...
              <div className="relative group">
                <div
                  className={`w-3 h-3 ${getStatusColor(status)} rounded-full cursor-pointer`}
                ></div>
        ...
        <div className="overflow-y-auto h-full">
          <div className="p-6 flex flex-col items-center justify-start min-h-full">
            ...
            <div className="w-full max-w-2xl mb-12">
              ...
              <SearchBar
                placeholder="Search users or dial"
                    ...
                    {contacts.map((user) => (
                      ...
                      >
                        ...
                        <div className="flex gap-2">
                          <button
                            onClick={() =>
                              handleCall({ id: user.id, name: user.name })
                            }
                          ...
                          <button
                            onClick={() =>
                              handleToggleFavorite(user.id, user.name)
                            }
                  ...
                  <div className="grid gap-3">
                    {favorites.map((fav) => (
                      ...
                      >
                        ...
                        <button
                          onClick={() =>
                            handleCallFavorite(fav.userId, fav.username)
                          }
